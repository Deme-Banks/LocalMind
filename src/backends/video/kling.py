...
class KlingVideoBackend(BaseVideoBackend):
    """Backend for Kling AI video generation"""
    
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.api_key = config.get("api_key") or os.getenv("KLING_API_KEY")
        self.base_url = config.get("base_url", "https://api.klingai.com/v1")
        self.headers = {
    ...
    def list_models(self) -> list[str]:
        """List available Kling AI models"""
        return [
            "kling-v1",
            "kling-v1.1"
        ]
    ...
    def generate_video(
        ...
        **kwargs
        ...
        if not self.is_available():
            raise ConnectionError("Kling AI API key not configured.")
        ...
        try:
            ...
            logger.error(f"Kling AI video generation failed: {e}")
            raise
    ...
    def get_video_status(self, video_id: str) -> Optional[VideoResponse]:
        ...
        try:
            ...
            logger.error(f"Kling AI video status check failed for {video_id}: {e}")
            return None
